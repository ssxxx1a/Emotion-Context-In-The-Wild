...
class Norm(nn.Module):
    ...
    def forward(self, x):
        ...
        norm = self.alpha * (x - x.mean(dim=-1, keepdim=True)).div((x.std(dim=-1, keepdim=True) + self.eps)) + self.bias
...
class TX(nn.Module):
    def __init__(self, d_model=16, dropout=0.3):
...
class Tail(nn.Module):
    def __init__(self, num_classes, num_frames, head=16):
        ...
        for i in range(self.head):
            self.head_layers.append(Block_head(self.d_model // self.head))
...
if __name__ == '__main__':
    x = torch.rand(size=(16, 16, 3, 112, 112))
    Net = Semi_Transformer(num_classes=7, seq_len=16)
    print(Net(x)[1].size())
