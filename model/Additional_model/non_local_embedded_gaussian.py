...
class _NonLocalBlockND(nn.Module):
    ...
    def forward(self, img, face, return_nl_map=False):
        """
        :param x: (b, c, t, h, w)
        ...
        g_x = self.g(face).view(batch_size, self.inter_channels, -1)
        g_x = g_x.permute(0, 2, 1)

        theta_x = self.theta(img).view(batch_size, self.inter_channels, -1)
        theta_x = theta_x.permute(0, 2, 1)
        phi_x = self.phi(face).view(batch_size, self.inter_channels, -1)
        f = torch.matmul(theta_x, phi_x)
        f_div_C = F.softmax(f, dim=-1)

        y = torch.matmul(f_div_C, g_x)
        y = y.permute(0, 2, 1).contiguous()
        y = y.view(batch_size, self.inter_channels, *face.size()[2:])
        W_y = self.W(y)
      #  z = W_y + face
        z=W_y
