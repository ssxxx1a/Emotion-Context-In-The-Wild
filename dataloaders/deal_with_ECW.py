                  ...
                  #  print(emotion)
                    ...
                    for _index, _frame in enumerate(range(2 * fps + 2, frames_num - fps + 1)):
                        ...
                        if sucess:
                            ...
                            cv2.imwrite(os.path.join(save_path, emotion, dir, 'img', str(_index).zfill(3)+'.jpg'), img)
                            cv2.imwrite(os.path.join(save_path, emotion, dir, 'face', str(_index).zfill(3)+'.jpg'), face)
    ...
    for k,v in emotion_path_list.items():
        ...
        for name in dataset_name:
            ...
            for train_item in v[:l]:
                train_cmd='mv  '+os.path.join(train_item,name)+' '+os.path.join(save_path,name,'train',k)+'/S'+str(train_count).zfill(4)
                os.system(train_cmd)
                train_count+=1
            test_count=0
            for test_item in v[l:]:
                test_cmd='mv  '+os.path.join(test_item,name)+' '+os.path.join(save_path,name,'test',k)+'/S'+str(test_count).zfill(4)
                test_count+=1
                os.system(test_cmd)
